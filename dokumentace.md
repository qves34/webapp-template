...
## Bannery na Přátelích/Profilu, zoom celého panelu, oprava po vyzkoušení (2026-08-21, pokračování)

Rychlé iterace po nasazení výše popsaného:

- **Bannery se dřív renderovaly jen v hlavním pohledu ("Moje")** - `<SideBanners>` byla jen v posledním `return` v `Watchlist`. Na žádost uživatele doplněná i do zbylých 3 návratů (`viewingFriend` - watchlist přítele, `view === 'friends'`, `view === 'profile'`), každý teď obalený stejným fragmentem `<><SideBanners .../><main>...</main></>`.
- **Zkoušel se `zoom: 1.25` na `:root`** (celá appka o 25 % větší, jako browser zoom, co si uživatel oblíbil) - **rozbilo to `.side-banner--custom`**: `100vw` se pod zoomem na kořenovém elementu chová nekonzistentně napříč `position: fixed` prvky, banner pak zabíral přes půl obrazovky a přesahoval přes hlavičku appky. Vráceno, doporučeno použít zoom přímo v prohlížeči (ten se navíc ukládá per-web automaticky).
- **Zoom natvrdo zkusen znovu, ale cíleněji**: nová proměnná `--app-zoom: 1.15` (index.css), `zoom: var(--app-zoom)` jen na `.app` a `.controls` (ne `:root`) v `@media (min-width: 1400px)` bloku - to je "normální" nested zoom na jednom prvku, ne na kořenovém, takže se nekříží se stejným problémem. `.side-banner--custom` výpočet upravený na `calc((100vw - (var(--app-max-width) * var(--app-zoom))) / 2)`, ať počítá se zvětšenou reálnou šířkou `.app`. Samostatný `zoom: 1.15` na `.appearance-panel` (karta Vzhled, přidaný o pár kroků dřív) odstraněný - zdědí zvětšení z `.controls` automaticky, dvojitý zoom by ji zvětšil na ~1.32×.

## Zrušen banner "Trendující", plán dalších funkcí (2026-08-21, pokračování)

Uživatel se zeptal na nápady na nové funkce. Navrženo (a odsouhlaseno k postupné realizaci): roční přehled/statistiky, živé oznámení o žádosti o přátelství (Realtime), rewatch tracking, PWA. Nápad na ruční "zamíchat" u trendujícího banneru zamítnut - místo toho **zrušená celá záložka "Trendující"** v panelu Vzhled (uživatel usoudil, že vlastní banner z konkrétního titulu je lepší hlavní vizuál) a plán na samostatnou budoucí stránku "Trendující" (top 5 filmů/seriálů/anime právě teď, ne banner).
...
### 4/4: PWA (instalovatelnost)

- **Ikony**: appka měla jen `favicon.svg`, manifest potřebuje PNG. Dočasně nainstalovaný `sharp` (`npm install --no-save`, stejný postup jako `pg` na DB migrace, po použití `npm uninstall`) vyrenderoval `icon-192.png`, `icon-512.png`, `apple-touch-icon.png` (180×180) přímo z `favicon.svg`, plus `icon-512-maskable.png` (obsah zmenšený na ~72 % a vycentrovaný na plné pozadí `--can`, ať OS maska - kruh/squircle - neuřízne rohy filmové cívky).
- **`public/manifest.webmanifest`** + **`public/sw.js`** (registrace v `main.jsx`, jen `import.meta.env.PROD`) - SW bez cachování, jen `fetch` handler kvůli podmínce instalovatelnosti (appka běží na živých datech ze Supabase, offline zápis vědomě mimo scope, viz README). `index.html` doplněný o `<link rel="manifest">`, `apple-touch-icon` a `apple-mobile-web-app-*` meta tagy pro iOS.
- **Vedlejší nález**: `npm run build` lokálně tiše nefungoval (exit 0, ale `dist/` se vůbec nevytvořil) - `node_modules/@esbuild/*` chybělo, pravděpodobně smazané při některém z dřívějších `npm uninstall pg`/`sharp` cyklů. Opraveno `npm ci`. Cestou se ukázalo, že moje dřívější `npm run build 2>&1 | tail -N; echo "BUILD: $?"` ověřování bylo vadné - `$?` v pipeline hlásí exit kód `tail`, ne `npm run build`, takže i skutečně rozbitý build by prošel jako "BUILD: 0". Stejná roura maskovala i reálné `oxlint` varování (`no-useless-escape` v `lib/markdown.js`, opraveno). Nasazení na Vercelu tím nebylo dotčené (Vercel builduje nezávisle na lokálním `node_modules`), jen moje lokální kontroly před pushem byly od nějakého bodu v týhle session naslepo. Napříště ověřovat přímo (`npm run build` bez roury do `tail`), ne přes `$?` za pipeline.
